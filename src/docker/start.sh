...
sed -i "s/{postgres-port}/${POSTGRES_PORT}/"            /usr/local/tomcat/conf/Catalina/localhost/broker.xml
...
sed -i "s%{mail-port}%${MAIL_PORT}%"                    /usr/local/tomcat/conf/mailSending.xml
sed -i "s%{mail-protocol}%${MAIL_PROTOCOL}%"            /usr/local/tomcat/conf/mailSending.xml
