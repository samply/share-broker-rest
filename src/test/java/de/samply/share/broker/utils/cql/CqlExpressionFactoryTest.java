...
class CqlExpressionFactoryTest {
  ...
  void test_getPathExpression_existing_shortPath() {
    String pathExpression = factory
        .getPathExpression(MDR_URN_SIMPLE, ENTITY_TYPE_PATIENT, "values-expression");
    assertThat("Error reading path expression with path '{0}'.", StringUtils.trim(pathExpression),
        is("values-expression"));
  }

  @Test
  void test_getPathExpression_existing_longPath() {
    String pathExpression = factory
        .getPathExpression(MDR_URN_SIMPLE, ENTITY_TYPE_SPECIMEN, "values-expression");
    assertThat("Error reading path expression with path including text.",
        StringUtils.trim(pathExpression), is("exists(from [Patient] P where values-expression)"));
  }

  @Test
  void test_getPathExpression_notExistingEntityType() {
    String pathExpression = factory
        .getPathExpression(MDR_URN_SIMPLE, ENTITY_TYPE_NOT_EXISTING, "values-expression");
    assertThat("Error getting path expression for non-existing entity type.",
        StringUtils.trim(pathExpression), is(""));
  }

  @Test
  void test_getPathExpression_notExistingMdrUrn() {
    String pathExpression = factory
        .getPathExpression(MDR_URN_NOT_EXISTING, ENTITY_TYPE_PATIENT, "values-expression");
    assertThat("Error getting path expression for non-existing MDR-urn.",
        StringUtils.trim(pathExpression), is(""));
  }
  ...
  void test_getAtomicExpression_operatorNotSpecifiedUseDefault() {
    ...
    List<CqlExpressionFactory.AtomicExpressionParameter> atomicExpressionParameterList = factory
        .createAtomicExpressionParameterList(MDR_URN_SIMPLE, ENTITY_TYPE_SPECIMEN,
            EssentialValueType.DECIMAL, valueDto);
    assertThat("Expected is a list with only one parameter", atomicExpressionParameterList.size(),
        is(1));
  ...
  void test_getAtomicExpression_operatorSpecified() {
    ...
    assertThat("Error reading atomic expression for specified operator.",
        StringUtils.trim(atomicExpression),
        is("(other-cql-expression < '17' and other-cql-expression >\n          '13')"));
  }
  ...
  void test_getAtomicExpression_notExistingMdrUrn() {
    EssentialSimpleValueDto valueDto = createValueDto(SimpleValueCondition.BETWEEN);
    List<CqlExpressionFactory.AtomicExpressionParameter> atomicExpressionParameterList = factory
        .createAtomicExpressionParameterList(MDR_URN_NOT_EXISTING, ENTITY_TYPE_SPECIMEN,
            EssentialValueType.DECIMAL, valueDto);
  ...
  void test_getAtomicExpression_notExistingEntityType() {
    EssentialSimpleValueDto valueDto = createValueDto(SimpleValueCondition.BETWEEN);
    List<CqlExpressionFactory.AtomicExpressionParameter> atomicExpressionParameterList = factory
        .createAtomicExpressionParameterList(MDR_URN_SIMPLE, ENTITY_TYPE_NOT_EXISTING,
            EssentialValueType.DECIMAL, valueDto);
  ...
  void test_getExtensionUrl_filled() {
    ...
    assertThat("Error getting extension name.", StringUtils.trim(extensionUrl),
        is("https://fhir.bbmri.de/StructureDefinition/url"));
  }
  ...
  void test_getCodesystems_filled_name() {
    ...
    assertThat("Error getting name of code system.", StringUtils.trim(codesystemName),
        is("SomeCodeSystem"));
  }
  ...
  void test_getCodesystems_filled_url() {
    ...
    assertThat("Error getting url of code system.", StringUtils.trim(codesystemUrl),
        is("https://fhir.bbmri.de/CodeSystem/url"));
  }
  ...
  void test_getCodesystems_filled_twoCodesystems_names() {
    ...
    Set<String> actualNames = codesystems.stream().map(CqlConfig.Codesystem::getName)
  ...
  void test_getCodesystems_filled_twoCodesystems_urls() {
    ...
    Set<String> actualUrls = codesystems.stream().map(CqlConfig.Codesystem::getUrl)
        .collect(Collectors.toSet());
  ...
  void test_getSingletons_empty_forEntityType() {
    Set<CqlConfig.Singleton> singletons = factory
        .getSingletons(MDR_URN_PATIENT, ENTITY_TYPE_PATIENT);
    assertThat("Error getting empty list for entity type without singletons.", singletons,
  ...
  void test_getSingletons_filled_forEntityType() {
    Set<CqlConfig.Singleton> singletons = factory
        .getSingletons(MDR_URN_PATIENT, ENTITY_TYPE_SPECIMEN);
  ...
  void test_getSingletons_filled_twoSingletons() {
    Set<CqlConfig.Singleton> singletons = factory
        .getSingletons(MDR_URN_PATIENT_OBSERVATION, ENTITY_TYPE_SPECIMEN);
    ...
    Set<String> actualNames = singletons.stream().map(CqlConfig.Singleton::getName)
        .collect(Collectors.toSet());
    assertThat("Error getting names of list with 2 singletons.", actualNames,
        is(expectedSingletons));
  }

  @Test
  void test_getSingletons_filled_twoIdenticalSingletons() {
    Set<CqlConfig.Singleton> singletons = factory
        .getSingletons(MDR_URN_PATIENT_PATIENT, ENTITY_TYPE_SPECIMEN);
  ...
  void test_getCqlValueList_configured() {
    ...
    assertThat("Error getting cql value for permitted value in config file.",
        StringUtils.trim(cqlValue), is("cqlCoding2"));
  }
  ...
  void test_getCqlValueList_notConfigured() {
    ...
    String cqlValue = cqlValueList.get(0);
    assertThat("Error getting cql value for permitted value not configured in config file.",
        StringUtils.trim(cqlValue), is(not_configured));
  }
  ...
  void test_getCqlValueList_twoAssociatedCqlValues() {
    ...
    List<String> cqlValues = cqlValueList.stream().map(StringUtils::trim).sorted()
        .collect(Collectors.toList());
    List<String> expectedCqlValues = Arrays.asList("SMOKER_1", "SMOKER_2");

    assertThat("Error getting cql value for permitted value not configured in config file.",
        cqlValues, is(expectedCqlValues));
  }
