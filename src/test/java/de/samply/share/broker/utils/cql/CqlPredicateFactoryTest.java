...
class CqlPredicateFactoryTest {
  ...
  void test_create_oneFieldWithoutAtomicExpression() {
    ...
    assertThat("No atomic expression should result in trivial predicate 'true'", predicate,
        is("true"));
  }
  ...
  void test_create_oneField() {
    ...
    expect(fieldExpressionFactory.create(eq(MDR_URN_1), eq(ENTITY_TYPE), anyObject()))
        .andReturn("field-expression-1");
  ...
  void test_create_twoFields() {
    ...
    expect(fieldExpressionFactory.create(eq(MDR_URN_1), eq(ENTITY_TYPE), anyObject()))
        .andReturn("field-expression-1");
    expect(fieldExpressionFactory.create(eq(MDR_URN_2), eq(ENTITY_TYPE), anyObject()))
        .andReturn("field-expression-2");

    replay(fieldExpressionFactory);

    String predicate = predicateFactory.create(queryDto, ENTITY_TYPE);
    assertThat("Two field (in one entity) both with atomic expression.", predicate,
        is("field-expression-1 and field-expression-2"));
  }
  ...
  void test_create_twoFieldsInTwoEntites() {
    ...
    String predicate = predicateFactory.create(queryDto, ENTITY_TYPE);
    assertThat("Two field (in two entites) both with atomic expression.", predicate,
        is("field-expression-1 and field-expression-2"));
  }
  ...
  void test_create_twoFieldsOneWithoutAtomicExpression() {
    ...
    String predicate = predicateFactory.create(queryDto, ENTITY_TYPE);
    assertThat("Two field one with and one without atomic expression.", predicate,
        is("field-expression-2"));
  }
  ...
  void test_create_twoFieldsBothWithoutAtomicExpression() {
    ...
    String predicate = predicateFactory.create(queryDto, ENTITY_TYPE);
    assertThat("No atomic expression in all fields should result in trivial predicate 'true'",
        predicate, is("true"));
  }
}
