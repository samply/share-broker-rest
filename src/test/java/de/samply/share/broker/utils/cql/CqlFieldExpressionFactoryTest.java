...
class CqlFieldExpressionFactoryTest {
  ...
  void initFactory() {
    ...
    fieldExpressionFactory = new CqlFieldExpressionFactory(expressionFactory,
        valuesExpressionFactory);
  }

  @Test
  void test_create_emptyValueExpression() {
    expect(valuesExpressionFactory.create(anyString(), anyString(), anyObject())).andReturn(null);
    expect(expressionFactory.getPathExpression(anyString(), anyString(), anyString()))
        .andReturn("path-expression");
    ...
    assertThat("Error creating field expression with missing values expression.", atomicExpression,
        nullValue());
  }

  @Test
  void test_create_filledValueExpression() {
    expect(valuesExpressionFactory.create(anyString(), anyString(), anyObject()))
        .andReturn("values-expression");
    expect(expressionFactory.getPathExpression(anyString(), anyString(), anyString()))
        .andReturn("path-expression");
    ...
    String atomicExpression = fieldExpressionFactory.create(MDR_URN_1, ENTITY_TYPE, fieldDto);
    assertThat("Error creating field expression with filled values expression.", atomicExpression,
        is("path-expression"));
  }
