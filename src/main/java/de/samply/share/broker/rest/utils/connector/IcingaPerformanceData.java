...
import de.samply.share.broker.utils.connector.PerformanceDataAdapter;
...
public class IcingaPerformanceData {
    ...
    public String toString() {
        ...
        if (critThreshold != null || warnThreshold != null || minThreshold != null || maxThreshold != null) {
    ...
    public static IcingaPerformanceData parse(String in) {
        try {
            ...
            if (uomIndex > 0) { // If the last unit is not a decimal, it contains an UOM
                icingaPerformanceData = new IcingaPerformanceData(name, value.substring(0, uomIndex), UnitOfMeasure.findByValue(value.substring(uomIndex)));
            } else {
