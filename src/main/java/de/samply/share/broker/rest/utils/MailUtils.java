...
import de.samply.share.broker.utils.WebUtils;
...
public class MailUtils {
    private static final Logger logger = LogManager.getLogger(de.samply.share.broker.utils.MailUtils.class);
    ...
    public static void checkAndSendReminders() {
        ...
        if (!SamplyShareUtils.isNullOrEmpty(reminders)) {
            for (Action reminder : reminders) {
                de.samply.share.broker.utils.MailUtils.sendReminder(Config.instance.getProperty(CCP_OFFICE_MAIL), reminder);
            }
    ...
    public static void checkAndSendRemindersExternal() {
        ...
        if (!SamplyShareUtils.isNullOrEmpty(reminders)) {
            for (Action reminder : reminders) {
                ...
                de.samply.share.broker.utils.MailUtils.sendReportReminder(project, reminder);
            }
            logger.debug("Sent " + reminders.size() + " reminders.");
        }
    }
    ...
    public static void checkAndSendNotifications() {
        ...
        if (inquiries == null || inquiries.size() < 1) {
            ...
            for (Inquiry inquiry : inquiries) {
                de.samply.share.broker.utils.MailUtils.sendExpiryWarning(inquiry);
            }
    ...
    public static void checkAndSendReportReminders() {
        ...
        if (!SamplyShareUtils.isNullOrEmpty(projects)) {
            for (Project project : projects) {
                de.samply.share.broker.utils.MailUtils.sendProjectEndingInfo(project);
            }
