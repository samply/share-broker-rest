...
class CqlValuesExpressionFactory {
  ...
  String create(String mdrUrn, String entityType, EssentialSimpleFieldDto fieldDto) {
    ...
    for (EssentialSimpleValueDto valueDto : fieldDto.getValueDtos()) {
      CollectionUtils.addAll(atomicExpressions,
          createSingleAtomicExpressionListForOneValueDto(mdrUrn, entityType,
              fieldDto.getValueType(), valueDto));
    }
    ...
    if (atomicExpressions.isEmpty()) {
      ...
      return "(" + StringUtils.join(atomicExpressions, " or ") + ")";
    }
  }

  private List<String> createSingleAtomicExpressionListForOneValueDto(String mdrUrn,
      String entityType, EssentialValueType valueType, EssentialSimpleValueDto valueDto) {
    List<CqlExpressionFactory.AtomicExpressionParameter> atomicExpressionParameterList =
        cqlExpressionFactory.createAtomicExpressionParameterList(mdrUrn, entityType, valueType,
            valueDto);
